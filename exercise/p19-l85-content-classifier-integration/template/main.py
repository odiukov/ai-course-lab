...
class Router:
    ...
    def _apply_redactors(self, text: str, verdicts: list[ClassifierVerdict]) -> str:
        """Последовательно примените редактирование каждого классификатора, для которого существует сработавший одноимённый вердикт."""
    ...
    def decide(self, text: str, verdicts: list[ClassifierVerdict]) -> Action:
        """Выберите максимальную тяжесть среди вердиктов и сформируйте Action по политике block, redact, warn или log вместе с сериализованными вердиктами и метаданными."""
        raise NotImplementedError
