...
class MLPProjector(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Спроецируй вход через первый линейный слой, GELU и второй линейный слой, сохранив ведущие измерения тензора."""
...
class MockTextEmbedding(nn.Module):
    ...
    def forward(self, ids: torch.Tensor) -> torch.Tensor:
        """Преобразуй батч идентификаторов в эмбеддинги и усредни только непустые токены, игнорируя padding с идентификатором 0."""
        raise NotImplementedError


def make_pair(seed: int, vocab_size: int, max_len: int) -> tuple[torch.Tensor, torch.Tensor]:
    """Создай детерминированную синтетическую пару: изображение заданного seed и дополненную нулями последовательность идентификаторов токенов допустимого диапазона."""
    raise NotImplementedError


def cosine_alignment_loss(image_emb: torch.Tensor, text_emb: torch.Tensor) -> torch.Tensor:
    """Вычисли среднюю попарную потерю 1 минус косинусная близость, предварительно нормализовав эмбеддинги по последнему измерению."""
    raise NotImplementedError
