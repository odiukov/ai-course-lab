...
class Dispatcher:
    ...
    async def dispatch(
        ...
        *,
        ...
        """Реализуйте полный вход диспетчера: поиск и валидацию инструмента, проверку бюджета, дедупликацию по idempotency key, использование свежего кэша и ограничение конкурентного выполнения семафором."""
    ...
    async def _run_with_retries(
        ...
        """Реализуйте ограниченный цикл попыток с таймаутом каждой попытки, повтором transient-ошибок, повтором таймаутов только для идемпотентных инструментов и публикацией результата в кэш и in-flight future."""
        raise NotImplementedError
...
async def _invoke(handler: Callable[..., Any], args: dict) -> Any:
    """Реализуйте единый асинхронный вызов синхронных, асинхронных и возвращающих awaitable обработчиков с передачей аргументов по именам."""
    raise NotImplementedError
...
def _map_exception(exc: Exception, attempts: int) -> DispatchError:
    """Преобразуйте исключение обработчика в DispatchError и сохраните исходные поля ошибки, обёрнутой для ожидающих дубликатов."""
    raise NotImplementedError
