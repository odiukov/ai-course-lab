...
def fixed_window(doc_id: str, text: str, size: int = 400, overlap: int = 80) -> list[Chunk]:
    """Реализуй разбиение текста на окна заданного размера с перекрытием и проверкой допустимости аргументов."""
...
def sentence_chunks(doc_id: str, text: str, target: int = 500) -> list[Chunk]:
    """Реализуй упаковку целых предложений в чанки до целевого размера с сохранением исходных смещений."""
    raise NotImplementedError
...
def recursive_split(doc_id: str, text: str, target: int = 500,
    ...
    """Реализуй иерархическое разбиение по заданной последовательности разделителей с формированием непустых чанков."""
    raise NotImplementedError
...
def semantic_chunks(doc_id: str, text: str, similarity_threshold: float = 0.55,
    ...
    """Реализуй группировку соседних предложений по сходству с текущим семантическим центроидом и жёсткому лимиту длины."""
    raise NotImplementedError
...
def structural_markdown(doc_id: str, text: str) -> list[Chunk]:
    """Реализуй разбиение Markdown-документа по заголовкам, включая заголовок в соответствующий чанк и сохраняя смещения."""
    raise NotImplementedError
