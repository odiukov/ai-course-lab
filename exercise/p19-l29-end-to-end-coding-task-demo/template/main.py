...
class GateChain:
    ...
    def evaluate(self, call: ToolCall, ctx: GateContext) -> ChainOutcome:
        """Последовательно пропускай вызов инструмента через цепочку ворот, накапливай решения и немедленно прекращай проверку при первом отказе."""
...
class Sandbox:
    ...
    def run(self, argv: list[str]) -> SandboxResult:
        """Выполняй команду внутри корня проекта с проверкой denylist и path jail, тайм-аутом, ограничением вывода и корректным преобразованием ошибок в SandboxResult."""
        raise NotImplementedError
...
class CodingAgentPolicy:
    ...
    def next_action(self, last_obs: Observation | None) -> tuple[str, tuple[str, ...], str]:
        """Сопоставь каждому рабочему состоянию политики предусмотренный вызов инструмента с точными аргументами и payload, сохранив существующий контракт метода."""
        raise NotImplementedError

    def observe(self, tool: str, exit_code: int, text: str) -> None:
        """Реализуй переходы конечного автомата по результатам инструментов, включая раннюю остановку при успешных тестах, подготовку исправления и итоговую верификацию."""
        raise NotImplementedError
...
class AgentRun:
    ...
    def run(self) -> AgentRunReport:
        """Собери полный цикл агента: соблюдай бюджеты, проверяй каждый вызов воротами, исполняй разрешённые инструменты, записывай наблюдения и шаги, создавай связанные spans и формируй итоговый отчёт."""
        raise NotImplementedError
