...
describe("адрес практики: файл + функция", () => {
  ...
  // Файл существует в упражнении, но не в НЁМ объявлена указанная функция —
  // это другая ошибка, чем «файла нет вовсе»: main.py в упражнении есть, а
  // fire объявлена только в hooks.py.
  it("ругается на файл, в котором этой функции нет, и подсказывает, где она есть", () => {
    const errors = validatePlan(
      [
        { id: "001-a", type: "theory", title: "Т" },
        { id: "002-b", type: "code", title: "К", exercise_fn: "fire", exercise_file: "main.py" },
      ],
      multiSource,
    );
    expect(errors).toContain(
      "Шаг 002-b: в файле main.py нет функции fire — она есть в hooks.py",
    );
  });
  ...
  // Обратная совместимость: у одно-файлового упражнения (files.length === 1)
  // текст «уже занята» не должен обзаводиться именем файла — оно там ничего
  // не различает, а старые сообщения обещаны неизменными буквально. Именно
  // этот тест поймал бы регрессию, где имя файла добавляется всегда.
  it("«уже занята» для одно-файлового упражнения — текст ошибки как раньше", () => {
    const plan = [
      step({ id: "001-t", type: "theory" }),
      step({ id: "002-c", type: "code", exercise_fn: "transpose" }),
      step({ id: "003-t", type: "theory" }),
    ...
    expect(validatePlan(plan, SOURCE)).toContain(
      "Шаг 004-c: функция transpose уже занята другим шагом",
    );
  });
});
