...
export function snapshot(storage: StorageLike, prefixes: string[]): StorageSnapshot {
  ...
  try {
    ...
    // Если хранилище отказало посреди чтения, неполный снимок хуже пустого:
    // слияние перепишет список прочитанного по тому, что успело прочитаться,
    // и шаги, не попавшие ни в снимок, ни в облако, потеряют отметку. Пустой
    // снимок означает «сливать нечего», и страница просто живёт дальше.
    return {};
  }
  return result;
}
