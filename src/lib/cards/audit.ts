...
/**
 * Существительные, называющие часть шага или курса, на которую можно
 * сослаться дейктически («формула выше», «на графике выше»).
 *
 * Каждый корень anchored явным списком окончаний, а не открытым «[а-я]*»:
 * открытый хвост цеплял любое слово, начинающееся с корня — «примерно»,
 * «кодирование», «формулировка» — и в паре с «выше» ловил ровно тот же
 * брак, ради починки которого затеян весь файл: «примерно выше» —
 * обычное сравнение в тексте про вероятность, а не дейктика.
 */
const REFERABLE_NOUN =
  "(?:" +
  "формул(?:а|ы|у|е|ой)" + // формула/-ы/-у/-е/-ой
  "|пример(?:а|е|у|ом)?" + // пример/-а/-е/-у/-ом
  "|код(?:а|е|у|ом)?" + // код/-а/-е/-у/-ом
  "|таблиц(?:а|ы|у|е|ей)" + // таблица/-ы/-у/-е/-ей
  "|рисун(?:ок|ка|ке|ку|ком)" + // рисунок/-ка/-ке/-ку/-ком (беглая «о»)
  "|график(?:а|е|у|ом)?" + // график/-а/-е/-у/-ом
  "|диаграмм(?:а|ы|у|е|ой)" + // диаграмма/-ы/-у/-е/-ой
  "|схем(?:а|ы|у|е|ой)" + // схема/-ы/-у/-е/-ой
  "|раздел(?:а|е|у|ом)?" + // раздел/-а/-е/-у/-ом
  ")";

/**
 * Объект сравнения, после которого «<существительное> выше/ниже» — обычное
 * сравнение, а не дейктика.
 *
 * У этой строки уже была прошлая жизнь: версия правила до сужения
 * REFERABLE_NOUN несла ровно один вырез, `(?!\s+нул)`, — именно на случай
 * «график выше нуля». Сужение корней до явных окончаний (коммит ab733c95)
 * этот вырез не перенесло, и «где график выше нуля» — канонический вопрос
 * фазы про математику — стало ошибочно заворачиваться. Список короткий и
 * открытый для расширения, а не потому что других объектов сравнения не
 * бывает: «нуля/нулю», «оси/ось» (абсцисс/ординат) и «среднего/средней» —
 * то, что реально встречается в текстах про знак функции, выпуклость и графики.
 */
const COMPARISON_OBJECT = "(?:нул[яю]?|ос[иь]|средн(?:его|ей))";
...
const DEICTIC: DeicticPattern[] = [
  { label: "в примере", pattern: /(?<![а-яА-ЯёЁ])в примере(?![а-яА-ЯёЁ])/i },
  {
    ...
    pattern: new RegExp(
      "(?<![а-яА-ЯёЁ])(?:" +
        ...
        // «формула/график/рисунок выше», но не «график выше нуля» —
        // сравнение с объектом, а не отсылка к тексту рядом.
        `|${REFERABLE_NOUN}\\s+выше(?!\\s+${COMPARISON_OBJECT})` +
        ")(?![а-яА-ЯёЁ])",
      "i",
    ),
  },
  {
    label: "ниже",
    pattern: new RegExp(
      // «как ниже» и «ниже по тексту» сюда намеренно не входят — в русском
      // это не идиома (в отличие от «как выше», «выше по тексту»), а не
      // недосмотр симметрии с паттерном «выше».
      "(?<![а-яА-ЯёЁ])(?:" +
        ...
        // «формула/график/рисунок ниже», но не «график ниже нуля».
        `|${REFERABLE_NOUN}\\s+ниже(?!\\s+${COMPARISON_OBJECT})` +
        ")(?![а-яА-ЯёЁ])",
      "i",
    ),
  },
