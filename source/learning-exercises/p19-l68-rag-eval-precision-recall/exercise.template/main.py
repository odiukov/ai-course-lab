...
def precision_at_k(retrieved: list[str], gold: set[str], k: int) -> float:
    """Вычислите precision@k по первым k идентификаторам документов, корректно обработав неположительный k и пустую выдачу."""
    raise NotImplementedError


def recall_at_k(retrieved: list[str], gold: set[str], k: int) -> float:
    """Вычислите recall@k как долю золотых документов, найденных среди первых k результатов, включая случай пустого gold-набора."""
    raise NotImplementedError
...
def mean_reciprocal_rank(retrieved_per_query: list[list[str]],
    ...
    """Вычислите среднее reciprocal rank по всем запросам и проверьте согласованность размеров списков выдач и эталонов."""
    raise NotImplementedError
...
def ndcg_at_k(retrieved: list[str], graded: dict[str, int], k: int) -> float:
    """Нормализуйте DCG первых k результатов относительно идеального порядка документов с учетом градуированной релевантности."""
    raise NotImplementedError
...
def faithfulness(claims: list[str], context_texts: list[str], judge: MockJudge) -> float:
    """Оцените долю утверждений ответа, которые судья считает подтвержденными объединенным контекстом извлеченных документов."""
    raise NotImplementedError
