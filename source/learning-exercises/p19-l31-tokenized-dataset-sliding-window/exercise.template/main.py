...
class SlidingWindowDataset(Dataset):
    ...
    def count_windows(num_ids: int, context_length: int, stride: int) -> int:
        """Вычисли число полных окон длиной context_length + 1 с заданным шагом, отбрасывая неполный хвост."""
        raise NotImplementedError

    def __len__(self) -> int:
        """Верни количество доступных полных окон для сохранённого потока токенов и параметров датасета."""
        raise NotImplementedError

    def __getitem__(self, index: int) -> tuple[torch.Tensor, torch.Tensor]:
        """Сформируй по индексу пару long-тензоров input и target длины context_length со сдвигом на один токен; поддержи отрицательные индексы и проверку границ."""
        raise NotImplementedError
...
def make_dataloader(
    ...
    """Собери DataLoader с переданными параметрами батча и детерминированным перемешиванием, используя сумму base_seed и epoch как seed."""
    raise NotImplementedError
