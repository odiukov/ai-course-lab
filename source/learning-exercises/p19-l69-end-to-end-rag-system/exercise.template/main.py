...
class Chunker:
    ...
    def chunk(self, doc_id: str, text: str) -> list[Chunk]:
        """Разбей текст выбранной рекурсивной стратегией и преобразуй непустые фрагменты в последовательно пронумерованные Chunk."""
...
class HybridIndex:
    ...
    def search(self, query: str, k_each: int = 8, k_out: int = 12) -> list[Chunk]:
        """Выполни лексический и плотный поиск, объедини их ранги через RRF и верни не более k_out чанков."""
        raise NotImplementedError
...
class Rewriter:
    def pick_strategy(self, query: str) -> str:
        """Выбери стратегию переписывания запроса по авторским эвристикам для составных, терминологических и обычных запросов."""
        raise NotImplementedError
...
def generate_answer(query: str, ranked: list[tuple[Chunk, float]]) -> tuple[str, list[str]]:
    """Сформируй ответ из лучших релевантных чанков с якорями цитат либо откажись отвечать при пустой выдаче или низкой уверенности."""
    raise NotImplementedError
...
class Pipeline:
    ...
    def query(self, question: str) -> Result:
        """Свяжи переписывание, гибридный поиск, переранжирование и генерацию в один запрос, измерив задержку каждого этапа и вернув Result."""
        raise NotImplementedError
