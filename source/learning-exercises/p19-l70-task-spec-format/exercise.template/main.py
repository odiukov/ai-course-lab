...
def validate_task(record: dict, line_number: int = 0) -> list[ValidationError]:
    """Реализуй строгую валидацию одной записи: последовательно проверяй обязательные и неизвестные поля, типы, допустимость пары категории и метрики, а также встроенные few-shot примеры, возвращая список ValidationError."""
    raise NotImplementedError


def validate_file(path: str) -> tuple[list[TaskSpec], list[ValidationError]]:
    """Реализуй построчную загрузку JSONL: продолжай после ошибочного JSON, применяй validate_task, отклоняй повторные task_id и преобразуй корректные записи в TaskSpec."""
    raise NotImplementedError


def render_prompt(task: TaskSpec) -> str:
    """Собери единый промпт из few-shot примеров и основного prompt, сохраняя порядок и разделяя части пустой строкой."""
    raise NotImplementedError
...
def post_process(text: str, rule: str) -> str:
    """Реализуй все закреплённые правила детерминированной постобработки текста и выбрасывай ValueError для неизвестного правила."""
    raise NotImplementedError
