...
LESSON_58 = PHASE_ROOT / "58-vision-encoder-patches" / "code"
...
class MultiHeadSelfAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, store_attn: bool = False) -> torch.Tensor:
        """Реализуй многоголовое self-attention: сформируй Q, K и V по головам, вычисли масштабированные веса внимания, при запросе сохрани их и верни результат выходной проекции."""
...
class Block(nn.Module):
    ...
    def forward(self, x: torch.Tensor, store_attn: bool = False) -> torch.Tensor:
        """Собери прямой проход pre-LN блока из attention и FFN, сохранив обе остаточные связи и передачу флага сохранения внимания."""
        raise NotImplementedError
...
class ViT(nn.Module):
    ...
    def forward(self, x: torch.Tensor, store_attn: bool = False) -> torch.Tensor:
        """Последовательно пропусти токены через все блоки ViT и примени финальную нормализацию."""
        raise NotImplementedError
...
class VisionEncoder(nn.Module):
    ...
    def forward(self, x: torch.Tensor, store_attn: bool = False) -> tuple[torch.Tensor, torch.Tensor]:
        """Соедини готовый фронтенд патчей со стеком ViT и верни контекстные токены вместе с первым CLS-вектором."""
        raise NotImplementedError
