...
def paired_t_test(candidate: list[float], baseline: list[float]) -> tuple[float, float | None, int]:
    """Реализуй парный t-тест по разностям наблюдений, включая случаи пустой выборки, одной пары, несовпадающих длин и нулевой дисперсии."""
...
def _pair_by_seed(
    ...
    """Сопоставь результаты кандидата и базовой конфигурации по уникальным seed и верни согласованные списки значений метрики в детерминированном порядке."""
    raise NotImplementedError


def _improvement(candidate_mean: float, baseline_mean: float, direction: str) -> float:
    """Вычисли знаковое относительное улучшение с учётом направления метрики и корректно обработай нулевое базовое значение."""
    raise NotImplementedError


def _log_transform(values: list[float], scale: str) -> list[float]:
    """Преобразуй положительные значения логарифмической метрики натуральным логарифмом, сохраняя линейные значения без изменения."""
    raise NotImplementedError
...
class Evaluator:
    ...
    def _verdict(self, improvement: float, p_value: float | None, n: int) -> tuple[str, str]:
        """Реализуй выбор improved, regressed или noise по порогу улучшения, наличию и значимости p-value; верни также однострочное обоснование."""
        raise NotImplementedError
