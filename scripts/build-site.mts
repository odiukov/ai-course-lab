...
async function main(): Promise<void> {
  ...
  for (const slug of lessonSlugs()) {
    ...
    // Упражнение копируется целиком: заготовка, тесты и эталон — все файлы,
    // не только тот, что открыт в редакторе шага. Тесты импортируют из файла
    // все функции сразу, поэтому в браузере должен лежать весь файл (а для
    // каталожной формы — весь набор файлов), а не одна функция шага.
    // `exerciseFiles` сама решает раскладку: одно-файловая форма — плоско,
    // как раньше, каталожная — в подкаталоги `template/` и `solution/`.
